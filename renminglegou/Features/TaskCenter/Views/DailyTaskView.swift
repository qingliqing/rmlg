        ...
        if let taskInfo = viewModel.taskInfo {
            ZStack(alignment: .top) {
                ...
                VStack(spacing: 16) {
                    ...
                    HStack(spacing: 15) {
                        ForEach(0..<5, id: \.self) { index in
                            ZStack (alignment: .top){
                                // 根据任务完成状态显示不同图片
                                Group {
                                    if index < taskInfo.completedCount {
                                        // 已完成状态
                                    ...
                                    } else if index == taskInfo.completedCount && taskInfo.canReceiveReward {
                                        // 可领取状态
                    ...
                    // 完成按钮
                    Button(action: {
                        if taskInfo.canReceiveReward && !viewModel.isReceiving {
                    ...
                    .disabled(!taskInfo.canReceiveReward || viewModel.isReceiving)
                    .padding(.horizontal, 20)
                    .padding(.bottom, 20)
                }
            }
